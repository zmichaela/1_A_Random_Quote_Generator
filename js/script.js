...
// Created an array with 7 objects (quotes)
let quotes = [
  {
...
function printQuote () {
let storeObject = getRandomQuote(); // stores a random number generated by the above function
let htmlString = '<p class="quote">' + storeObject.quote + '</p>';
htmlString += '<p class="source">' + storeObject.source + '</p>';
if (storeObject.citation) {
    htmlString += '<p>' + storeObject.citation + '</p>';
};
if (storeObject.year) {
    htmlString += '<p>' + storeObject.year + '</p>';
};
return htmlString; 
}
